...
/** Wall ladder. Origin at the bottom centre; climbs +Y. Stiles at ±X, rungs across X, 6 cm off the wall. */
export function ladderRun(A, pm, h, opts = {}) {
  const w = opts.w ?? 0.56;
  ...
  const rise = h / n;
  for (const sx of [-1, 1]) {
    A.add(key, bar, LL(pm, sx * (w / 2), h / 2, 0.06, 0, 0.04, h, 0.04), {
      masks: [0.8, 0.4, 0.15],
    });
  }
  for (let i = 1; i < n; i++) {
    A.add(key, bar, LL(pm, 0, i * rise, 0.06, 0, w, 0.03, 0.03), {
      masks: [0.75, 0.4, 0.15],
    });
  }
}
