...
| `src/weapons/` | `viewmodel.js` optics, `radio-mesh.js` shader; `index.js` hands off weapon scene, `preview.js` uses WebGL renderer. | Node optics and viewmodel outside world TAA/fog, separate MSAA; preview uses WebGPU. |
...
- [x] Standalone WebGPU-only boot slice: strict device creation, no WebGL context on missing GPU/adapter, async init, world and separate 4× MSAA view pass, GPU readback, resize and disposal (`node tools/webgpu-boot/run.mjs` on a GPU-backed full Chromium). Unsupported-device checks also run in `npm test` without a GPU.
...
- [ ] Node materials and procedural texture forge, soldier/world/weapon representative captures; no migrated shader string surgery.
