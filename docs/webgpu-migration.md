...
- [x] Periodic TSL gradient noise and the four-band macro surface have a GPU probe on both real GPUs: seam periodicity, RGBA data packing (including fine detail in alpha), and spatial variation. This is a building block, **not** a production texture forge or material-system switch.
