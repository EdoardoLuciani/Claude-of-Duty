...
- [x] Periodic TSL gradient noise and the four-band macro surface have a GPU probe on both real GPUs: seam periodicity, RGBA data packing (including fine detail in alpha), spatial variation, and a linear RGBA8 mipmapped render-target bake/readback. This is a building block, **not** a production material-system switch.
